...
class Tiedb(Mydb):
    def __init__(self):
        Mydb.__init__(self, 'root', 'doreso1qaz2wsx', 'music_meta')
    ...
    def get_song_info(self, song, artist):
        return self._query_row('select album_id,album,musicUrl,lyricid,song_id from song_info where title=%s and artist=%s limit 1', (song, artist))
...
if __name__ == "__main__":
    ...
    for lrow in list(reader):
        ...
        try:
            song_info=mydb.get_song_info(lrow[0].decode('gbk').encode('UTF-8'),lrow[1].decode('gbk').encode('UTF-8'))
